...
if [ $rc -eq 0 ] \
	...
	&& [ "$age_note_ok" -eq 1 ] \
	&& [ "$(git config -f "$sandbox/home/.config/git/local" --get user.email)" = "you@example.com" ] \
	...
	&& grep -q '^    wallpaper_path = "/home/user/Pictures/wall.jpg"$' "$cfg"; then
